...
export class Modrinth implements Packet {
    ...
    constructor(upstream: string, values: string[], flags: Set<string>) {
        ...
        // Updates data fields
        if(this.values.length < 4 || this.values[0] !== "modrinth")
            throw new Error(`Invalid upstream ${glow(this.upstream)}, expects ${glow("modrinth;$LABEL;$PLATFORM;$VERSION")}.`);
        [ this.label, this.platform, this.version ] = this.values.slice(1);
    }
    ...
    async grab(): Promise<{
        ...
        if(!response.ok) throw new Error(`Cannot find upstream ${glow(this.upstream)} on Modrinth.`);

        // Parses response
        const versions = (await response.json() as Version[])
            .filter((version) => version.files.length > 0)
            .sort((a, b) => +new Date(b.date_published) - +new Date(a.date_published));
        if(versions.length === 0) throw new Error(`Upstream ${glow(this.upstream)} is not supported on Modrinth.`);
    ...
    async sync(): Promise<void> {
        ...
        if(hash !== data.hash) {
            await file.delete();
            throw new Error(`Upstream ${glow(this.upstream)} leads to a corrupted file.`);
        }
    ...
    async test(): Promise<void> {
        ...
        // Checks data
        const data = await this.grab().catch(() => null);

        // Suggests alternative
        if(data === null) {
            bad("TODO: Suggest alternative.");
            return;
        }
